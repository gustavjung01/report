...
function installStyle() {
  if (document.querySelector('style[data-mcp-route-admin]')) return;
  ...
  style.textContent = `
    ...
    .mcp-route-admin-row{display:grid!important;grid-template-columns:1fr 1fr!important;gap:6px!important;margin:0 0 8px!important}
    .mcp-route-admin-row button{min-height:34px!important;border-radius:11px!important;font-size:11px!important;font-weight:900!important}
    #modal[data-type="mcp-route-admin"]{width:min(420px,calc(100vw - 20px));max-height:calc(100dvh - 20px)}
    #modal[data-type="mcp-route-admin"] .modal{max-height:calc(100dvh - 20px)!important;overflow:hidden!important;display:grid!important;gap:10px!important}
    .mcp-route-admin-body{min-height:0!important;overflow:auto!important;display:grid!important;gap:9px!important;padding-right:2px!important}
    .mcp-route-admin-tools{display:grid!important;grid-template-columns:1fr 1fr!important;gap:8px!important}
    .mcp-route-admin-tools input,.mcp-route-admin-tools select{width:100%!important;min-height:38px!important;border:1px solid #cad7d4!important;border-radius:12px!important;background:#fff!important;padding:8px 10px!important;font:inherit!important;font-size:13px!important;box-sizing:border-box!important}
    .mcp-route-admin-card{border:1px solid #dce8e5!important;border-radius:14px!important;background:#fbfffd!important;padding:10px!important;display:grid!important;gap:7px!important}
    .mcp-route-admin-card h3{margin:0!important;font-size:14px!important;line-height:1.15!important}
    ...
    .mcp-route-customer-list{display:grid!important;gap:7px!important}
    .mcp-route-customer-row{border:1px solid #e6efec!important;border-radius:12px!important;background:#fff!important;padding:8px!important;display:grid!important;gap:7px!important}
    .mcp-route-customer-row.is-inactive{opacity:.58!important;background:#f8fafc!important}
    .mcp-route-customer-head{display:flex!important;justify-content:space-between!important;gap:8px!important;align-items:start!important;min-width:0!important}
    .mcp-route-customer-head b{font-size:13px!important;line-height:1.2!important;word-break:break-word!important}
    .mcp-route-customer-actions{display:grid!important;grid-template-columns:1fr 1fr!important;gap:6px!important}
    .mcp-route-customer-actions button,.mcp-route-customer-actions select{min-height:32px!important;border-radius:10px!important;font-size:11px!important;font-weight:900!important}
    .mcp-route-customer-actions select{border:1px solid #cad7d4!important;background:#fff!important;padding:5px 7px!important;min-width:0!important}
...
function ensureButtons() {
  ...
  if (shell && mcpTab && !shell.querySelector('[data-mcp-route-admin]')) {
    const row = shell.querySelector('.mcp-route-export-row') || shell.querySelector('.data-shell-open-card');
    row?.insertAdjacentHTML('afterend', '<div class="mcp-route-admin-row"><button type="button" class="secondary" data-mcp-route-admin>Quản trị tuyến</button><button type="button" class="secondary" data-mcp-import-customers>Import tuyến/khách</button></div>');
  }
}
...
function customerRow(customer, route, routes) {
  ...
  return `<article class="mcp-route-customer-row ${activeCustomer(customer) ? '' : 'is-inactive'}" data-route-customer-id="${esc(customer.id)}">
    <div class="mcp-route-customer-head"><div><b>${esc(customer.customer_name || 'Khách')}</b><small>${esc([customer.phone, customer.area, customer.address].filter(Boolean).join(' · ') || 'Chưa có thông tin')}</small><small>Tuyến: ${esc(routeLabel(route))}</small></div><small>${activeCustomer(customer) ? 'Đang bật' : 'Đang tắt'}</small></div>
    <div class="mcp-route-customer-actions">${routeSelect}<button type="button" data-mcp-admin-move-customer="${esc(customer.id)}">Chuyển tuyến</button><button type="button" class="secondary" data-mcp-admin-toggle-customer="${esc(customer.id)}">${activeCustomer(customer) ? 'Tắt khách' : 'Bật khách'}</button><button type="button" class="mcp-route-admin-danger" data-mcp-admin-hide-customer="${esc(customer.id)}">Ẩn mềm</button></div>
  </article>`;
}
...
async function renderAdminModal() {
  ...
  const routeId = dialog.querySelector('[data-mcp-admin-route-filter]')?.value || '';
  const query = norm(dialog.querySelector('[data-mcp-admin-search]')?.value || '');
...
async function toggleCustomer(customerId = '') {
  ...
  await putLocal(LOCAL_STORES.mcpRouteCustomers, { ...customer, active: nextActive, sync_status: 'local', updated_at: nowIso(), raw_payload: { ...(customer.raw_payload || {}), active_changed_at: nowIso() } });
...
async function hideCustomer(customerId = '') {
  ...
  await putLocal(LOCAL_STORES.mcpRouteCustomers, { ...customer, active: false, status: 'deleted', sync_status: 'local', deleted_at: customer.deleted_at || nowIso(), updated_at: nowIso(), raw_payload: { ...(customer.raw_payload || {}), deleted_at: customer.deleted_at || nowIso(), delete_reason: 'route_admin' } });
  await reorderRoute(customer.route_id);
  window.dispatchEvent(new CustomEvent('mcp:session-changed'));
  await renderAdminModal();
...
async function moveCustomer(customerId = '') {
  ...
  const targetRouteId = dialog?.querySelector(`[data-mcp-admin-target-route="${CSS.escape(customerId)}"]`)?.value || '';
  ...
  await putLocal(LOCAL_STORES.mcpRouteCustomers, { ...customer, route_id: targetRouteId, sort_order: maxOrder + 1, active: true, sync_status: 'local', updated_at: nowIso(), raw_payload: { ...(customer.raw_payload || {}), moved_at: nowIso(), moved_from_route_id: oldRouteId, moved_to_route_id: targetRouteId } });
...
document.addEventListener('input', (event) => {
  ...
  document.__mcpAdminSearchTimer = setTimeout(() => renderAdminModal().catch(console.warn), 260);
}, true);
