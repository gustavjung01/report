...
function uniqueCount(rows=[],keys=[]){const set=new Set();for(const row of rows){const v=val(row,keys);if(v)set.add(String(v).trim())}return set.size}
function fileKey(source,r={},i=0){
  ...
  const v=val(r,keys);return `${source}:${v||i}`;
}
function titleOfGroup(source,rows=[],i=0){
  const r=rows[0]||{};const date=dateOf(r);
...
function metaOfGroup(source,rows=[]){
  const r=rows[0]||{};const date=dateOf(r);const amount=money(first(rows,['total_amount','grand_total','amount','total']));
  ...
  const parts=[date,rows.length>1?`${rows.length} dòng`:'' ,customers?`${customers} khách`:'' ,products?`${products} sản phẩm`:'' ,amount].filter(Boolean);
  return parts.join(' · ')||sourceMeta[source]?.label||'File dữ liệu';
}
function groupRows(source,rows=[]){
  ...
  return Array.from(map.entries()).map(([id,group],i)=>({source,id,title:titleOfGroup(source,group,i),meta:metaOfGroup(source,group),date:dateOf(group[0]||{}),rows:group}));
}
async function collect(config){
  ...
  for(const source of config.sources){
    ...
    const rows=(await getAllLocal(meta.store)).filter(active);
    const groups=groupRows(source,rows);
    groups.sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
    data[source]=groups.slice(0,80);
    data[source].forEach(g=>state.flat.set(g.id,g));
  }
  return data;
}
...
function groupHtml(source,rows=[]){const meta=sourceMeta[source];return `<section class="ss-group" data-ss-source="${source}"><div class="ss-group-head"><b>${meta.icon} ${meta.label}</b><small>${rows.length} file</small></div><div class="ss-list">${rows.length?rows.map(row=>`<label class="ss-row"><input type="checkbox" data-ss-item data-source="${source}" value="${esc(row.id)}"><span><strong>${esc(row.title)}</strong><small>${esc(row.meta}</small></span></label>`).join(''):`<div class="ss-empty">Chưa có dữ liệu.</div>`}</div></section>`}
...
function showGenerating(snapshot){
  const modal=$('#modal');if(!modal)return;modal.dataset.type='smart-generating';
  modal.innerHTML=`<div class="ss-modal"><header class="ss-head"><h2>${esc(state.config?.title||'Báo cáo thông minh')}</h2><button class="ss-close" data-ss-close>Đóng</button></header><section class="ss-brief"><b>Đang tạo báo cáo</b><small>Chỉ phân tích ${snapshot.metrics.selected_files} file đã chọn.</small></section><div class="ss-body"><article class="ss-loading-card"><b>Đang phân tích dữ liệu...</b><small>File: ${snapshot.metrics.selected_files} · Dòng dữ liệu: ${snapshot.metrics.selected_rows}</small></article><section class="ss-result-card"><h3>Dữ liệu đã chọn</h3><div class="ss-selected-list">${snapshot.selected_items.map(x=>`<div>${esc(x.title)}<br><small>${esc(x.meta)}</small></div>`).join('')}</div></section></div><footer class="ss-foot"><button class="ss-secondary" data-ss-close>Đóng</button><button class="ss-primary" disabled>Đang tạo...</button></footer></div>`;
}
...
function showGenerated(payload){
  state.payload=payload;const r=payload.result||{};const modal=$('#modal');if(!modal)return;modal.dataset.type='smart-generated';
  modal.innerHTML=`<div class="ss-modal"><header class="ss-head"><h2>${esc(state.config?.title||'Báo cáo thông minh')}</h2><button class="ss-close" data-ss-close>Đóng</button></header><section class="ss-brief"><b>Kết quả phân tích</b><small>Đã phân tích ${payload.snapshot.metrics.selected_files} file đã chọn.</small></section><div class="ss-body"><article class="ss-result-card"><h3>Tổng quan</h3><p>${esc(r.summary||'Chưa có tóm tắt.')}</p></article><article class="ss-result-card"><h3>Khách / thị trường</h3>${list([...(r.market_insights||[]),...(r.customer_actions||[])],x=>typeof x==='string'?x:`${x.customer||'Khách'}: ${x.action||x.reason||''}`)}</article><article class="ss-result-card"><h3>Sản phẩm / cơ hội</h3>${list([...(r.product_insights||[]),...(r.order_opportunities||[])],x=>typeof x==='string'?x:`${x.product||x.customer||'Mục'}: ${x.insight||x.reason||''}`)}</article><article class="ss-result-card"><h3>Việc cần làm</h3>${list([...(r.next_steps||[]),...(r.risks||[])])}</article></div><footer class="ss-foot"><button class="ss-secondary" data-ss-save>Lưu báo cáo</button><button class="ss-primary" data-ss-export>Xuất TXT</button></footer></div>`;
}
...
async function createReport(){
  ...
  const snapshot=buildSnapshot(groups);showGenerating(snapshot);
  try{
    ...
    showGenerated(payload);toast(payload.ok?'Đã tạo xong báo cáo':'Chưa tạo được báo cáo');
  }catch(_e){showGenerated({ok:false,result:{summary:'Chưa tạo được báo cáo. Vui lòng thử lại sau.',next_steps:['Kiểm tra kết nối và thử lại.']},snapshot});toast('Chưa tạo được báo cáo')}
}
async function saveReport(){
  if(!state.payload)return toast('Chưa có báo cáo để lưu');await openLocalDb();
  ...
  await putLocal(LOCAL_STORES.aiSummaries,row);toast('Đã lưu báo cáo');
}
function exportTxt(){
  if(!state.payload)return toast('Chưa có báo cáo để xuất');const name=`${state.config?.file||'bao-cao-thong-minh'}-${state.payload.snapshot.today}.txt`;const blob=new Blob(['\ufeff'+resultText()],{type:'text/plain;charset=utf-8'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);toast('Đã xuất TXT');
}
...
document.addEventListener('click',async e=>{
  ...
  if(e.target.closest('[data-ss-export]')){e.preventDefault();exportTxt();return}
},true);
